  ...
  if (data.data.cartItems !== null) {
    ...
    const final = results.map((product) => {
      ...
      return {
        vendor: product.vendor,
        selected:
          product.products.length - countChange - countOutOfStocks - countOutOfStocks ===
          countSelection,
        totalDocs: product.products.length,
        products: d,
        count: countActive + countChange + countOutOfStocks,
    ...
    send = {
      ...
      totalPage: data?.data?.cartItems.totalPages,
      totalDocs: data?.data?.cartItems.totalDocs,
      unActive: unActive,
    };
  } else {
    send = {
